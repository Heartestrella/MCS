...
// frpcBin is the platform frpc executable name.
func frpcBin() string {
...
// ensureFrpc downloads the official frpc (fatedier/frp latest release) once.
func (m *Manager) ensureFrpc(hub *ConsoleHub) (string, error) {
	exe := filepath.Join(m.frpDir(), frpcBin())
	...
	// 官方发布命名: frp_x.x.x_{windows,linux,darwin}_{amd64,arm64}.{zip,tar.gz}
	arch := runtime.GOARCH
	plat := runtime.GOOS + "_" + arch
	var dlURL string
	for _, a := range rel.Assets {
		if strings.Contains(a.Name, plat) && (strings.HasSuffix(a.Name, ".zip") || strings.HasSuffix(a.Name, ".tar.gz")) {
			dlURL = a.URL
			break
		}
	}
	if dlURL == "" {
		return "", fmt.Errorf("未找到 %s 平台的 frpc 下载地址", plat)
	}
	ext := ".zip"
	if strings.HasSuffix(dlURL, ".tar.gz") {
		ext = ".tar.gz"
	}
	tmp := filepath.Join(m.frpDir(), "frp"+ext)
	...
	if err := extractArchive(tmp, exDir); err != nil {
		return "", fmt.Errorf("解压 frpc 失败: %w", err)
	}
	ms, _ := filepath.Glob(filepath.Join(exDir, "*", frpcBin()))
	if len(ms) == 0 {
		ms, _ = filepath.Glob(filepath.Join(exDir, frpcBin()))
	}
	if len(ms) == 0 {
		return "", fmt.Errorf("压缩包中未找到 %s", frpcBin())
	}
