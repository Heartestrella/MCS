...
// findJavaMajor looks for an existing java.exe of the given major:
// managed installs first (data/java/v<major>), then system installs.
func (m *Manager) findJavaMajor(major int) string {
	if ms, _ := filepath.Glob(filepath.Join(m.dataDir, "java", fmt.Sprintf("v%d", major), "*", "bin", "java.exe")); len(ms) > 0 {
		return ms[0]
	}
	// 旧版面板把 JRE21 解压在 data/java/ 根下
	if major == 21 {
		if ms, _ := filepath.Glob(filepath.Join(m.dataDir, "java", "*", "bin", "java.exe")); len(ms) > 0 {
			for _, p := range ms {
				if javaMajorOf(p) == 21 {
					return p
				}
...
// newest Temurin JRE zip of the given major (windows x64).
func tunaJREURL(major int) (string, error) {
	...
	re := regexp.MustCompile(fmt.Sprintf(`OpenJDK%dU-jre_x64_windows_hotspot_[0-9a-zA-Z._-]+\.zip`, major))
...
func (m *Manager) ensureJavaMajor(major int, hub *ConsoleHub) (string, error) {
	...
	sources = append(sources, dlSrc{"Adoptium 官方",
		fmt.Sprintf("https://api.adoptium.net/v3/binary/latest/%d/ga/windows/x64/jre/hotspot/normal/eclipse", major)})

	zipPath := filepath.Join(javaDir, "jre.zip")
	var lastErr error
	...
	if err := unzip(zipPath, javaDir); err != nil {
		os.Remove(zipPath)
		return "", fmt.Errorf("解压 Java 失败: %w", err)
	}
	os.Remove(zipPath)

	if ms, _ := filepath.Glob(filepath.Join(javaDir, "*", "bin", "java.exe")); len(ms) > 0 {
		hub.ClearProgress()
		hub.Broadcast(fmt.Sprintf("[MCS] Java %d 就绪: %s", major, ms[0]))
		m.notify(fmt.Sprintf("Java %d 自动安装完成", major),
			fmt.Sprintf("面板已自动下载并安装 Temurin JRE %d：\n%s", major, ms[0]))
		return ms[0], nil
	}
	return "", fmt.Errorf("Java 解压后未找到 java.exe")
}
