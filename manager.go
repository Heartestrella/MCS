...
func (m *Manager) launch(in *Instance, rs *runtimeState, dir string) {
	...
	if in.Type == "generic" {
		// 通用服务器：cmd /c 执行任意命令行（SteamCMD 装的游戏等）
		cmd = exec.Command("cmd", "/c", in.ExecCmd)
	} else {
		...
		if strings.EqualFold(filepath.Ext(in.JarFile), ".bat") {
			// Forge/NeoForge 新版：官方 run.bat 引导（内存参数写入 user_jvm_args.txt）
			var jvmArgs string
			...
			cmd = exec.Command("cmd", "/c", filepath.Join(dir, in.JarFile), "nogui")
			// run.bat 用 PATH 里的 java（不认 JAVA_HOME），把托管 Java 的 bin 排最前
			javaBin := filepath.Dir(javaPath)
			cmd.Env = append(os.Environ(),
				"PATH="+javaBin+string(os.PathListSeparator)+os.Getenv("PATH"),
				"JAVA_HOME="+filepath.Dir(javaBin))
		} else {
			var args []string
			if in.OptimizedFlags {
				args = aikarFlags(mem)
